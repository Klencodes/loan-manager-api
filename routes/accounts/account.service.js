...
async function forgotPassword({ email }, origin) {
    ...
    account.resetToken = {
        ...
        expires: new Date(Date.now() + 0*15*60*1000)
    };
    await account.save();
...
async function adminsOnly() {
    const admins = await db.Account.find({'role':'Admin'});
    if (!admins) throw 'Admin Account Lists not found';
    return admins;
}

async function usersOnly() {
    const users = await db.Account.find({'role':'User'});
    if (!users) throw 'User Account Lists not found';
    return users;
}
...
function basicDetails(account) {
    const { id, title, firstName, lastName, email, facebook, linkedin,phoneNumber, jobTitle, department, dob, address, city, state, country, role, created, updated, isVerified } = account;
    return { id, title, firstName, lastName, email,facebook, linkedin, phoneNumber, jobTitle, department, dob, address, city, state, country, role, created, updated, isVerified };
}
...
async function sendPasswordResetEmail(account, origin) {
    ...
    if (origin) {
        ...
        message = `<p>Please click the below link to reset your password, the link will be valid for 15 mins:</p>
